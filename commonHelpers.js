...
</li>`).join("")}function i(o){d.error({title:"Error",message:o})}function b(){u.innerHTML=""}function S(){f.classList.remove("hidden")}function $(){f.classList.add("hiden")}function q(){l=new g(".gallery a",{captionsData:"alt",captionDelay:250})}function v(){l?l.refresh():q()}const E=document.querySelector(".form-search"),c=document.querySelector('.form-search input[name="query"]');E.addEventListener("submit",P);function P(o){o.preventDefault();const e=c.value.trim();if(!e){i("Please enter a search term");return}b(),S(),p(e).then(n=>{n.length===0?i("Sorry, there are no images matching your search query. Please try again!"):(L(n),c.value="",v())}).catch(n=>{i("An error occured while fetching images"),console.error(n)}).finally(()=>{$()})}
